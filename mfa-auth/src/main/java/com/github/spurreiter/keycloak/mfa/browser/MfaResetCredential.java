...
public class MfaResetCredential implements Authenticator, AuthenticatorFactory {
    ...
    public void authenticate(AuthenticationFlowContext context) {
        ...
        try {
            ...
            // .setAuthenticationSession(authenticationSession).sendPasswordReset(link,
            // expirationInMinutes);

            MfaResponse response = MfaRequest.buildRequest(context).sendResetEmail(user.getAttributes(), link,
                    expirationInMinutes);
    ...
    public List<ProviderConfigProperty> getConfigProperties() {
        return ProviderConfigurationBuilder.create()
                ...
                .type(STRING_TYPE).defaultValue("http://localhost:1080/mfa/reset").add()
