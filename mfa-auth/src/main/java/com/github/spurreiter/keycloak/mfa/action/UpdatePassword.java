...
package com.github.spurreiter.keycloak.mfa.action;
...
public class UpdatePassword implements RequiredActionProvider, RequiredActionFactory, DisplayTypeRequiredActionFactory {
    ...
    private static final String LOGIN_UPDATE_FTL = "mfa-login-update-password.ftl";

    private String getPasswordPolicy (RequiredActionContext context) {
        Map<String, Object> map = new HashMap<>();
        PasswordPolicy passwordPolicy = context.getRealm().getPasswordPolicy();
        Set<String> policies = passwordPolicy.getPolicies();
        for(String policy : policies) {
            map.put(policy, passwordPolicy.getPolicyConfig(policy));
        }
        try {
    ...
    public void requiredActionChallenge(RequiredActionContext context) {
        Response challenge = context.form()
                ...
                .setAttribute("policy", getPasswordPolicy(context))
                .createForm(LOGIN_UPDATE_FTL);
    ...
    public void processAction(RequiredActionContext context) {
        ...
        EventBuilder errorEvent = event.clone().event(EventType.UPDATE_PASSWORD_ERROR).client(authSession.getClient())
        ...
        if (getId().equals(authSession.getClientNote(Constants.KC_ACTION_EXECUTING))
            ...
            session.sessions().getUserSessionsStream(realm, user)
                    ...
                    .collect(Collectors.toList()) // collect to avoid concurrent modification as backchannel Logout removes the user sessions.
        ...
        try {
            session.userCredentialManager().updateCredential(realm, user,
                    UserCredentialModel.password(passwordNew, false));
            context.success();
